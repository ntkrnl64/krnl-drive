...
export async function createSession(
  ...
  await db
    ...
    .bind(id, userId, pending2fa ? 1 : 0, now + ttl, now)
    .run();
  return id;
}
...
export async function promoteSession(
  ...
  const now = Date.now();
  await db
    .prepare("UPDATE sessions SET pending_2fa=0, expires_at=? WHERE id=?")
...
export async function listFiles(
  ...
  const { results } =
    parentId === null
      ? await db
          .prepare(
            "SELECT * FROM files WHERE parent_id IS NULL ORDER BY type DESC, name ASC",
          )
          .all<FileItem>()
      : await db
          .prepare(
...
export async function createFolder(
  ...
  await db
    ...
    .bind(id, name, parentId, "folder", 0, null, ownerId, now, now)
    .run();
...
export async function createFile(
  ...
  await db
    ...
    .bind(id, name, parentId, "file", size, mimeType, r2Key, ownerId, now, now)
    .run();
  return (await getFile(db, id))!;
}
...
export async function updateFile(
  ...
  values.push(id);
  await db
    .prepare(`UPDATE files SET ${parts.join(",")} WHERE id=?`)
    .bind(...values)
    .run();
}
...
export async function listSharesByFile(
  db: D1Database,
  fileId: string,
): Promise<Share[]> {
  const { results } = await db
...
export async function listAllShares(
  ...
  userId: string,
  role: string,
): Promise<(Share & { file_name: string })[]> {
  const { results } =
    role === "admin"
      ? await db
          .prepare(
            "SELECT shares.*, files.name as file_name FROM shares JOIN files ON shares.file_id = files.id ORDER BY shares.created_at DESC",
          )
          .all<Share & { file_name: string }>()
      : await db
          .prepare(
            "SELECT shares.*, files.name as file_name FROM shares JOIN files ON shares.file_id = files.id WHERE shares.created_by=? ORDER BY shares.created_at DESC",
          )
          .bind(userId)
          .all<Share & { file_name: string }>();
  return results;
...
export async function incrementShareView(
  db: D1Database,
  id: string,
): Promise<void> {
  await db
...
export async function incrementShareDownload(
  db: D1Database,
  id: string,
): Promise<void> {
  await db
...
export async function isDescendantOf(
  ...
  for (let i = 0; i < 64; i++) {
    const row = await db
      .prepare("SELECT parent_id FROM files WHERE id=?")
      .bind(current)
...
export async function updateUploadSession(
  ...
  id: string,
  updates: Partial<
    Pick<UploadSession, "uploaded_chunks" | "r2_upload_id" | "parts" | "status">
  >,
): Promise<void> {
  const parts: string[] = [];
  const values: unknown[] = [];
  ...
  values.push(id);
  await db
    .prepare(`UPDATE upload_sessions SET ${parts.join(",")} WHERE id=?`)
    .bind(...values)
    .run();
}
...
export async function getPasskeysByUser(
  db: D1Database,
  userId: string,
): Promise<Passkey[]> {
  const { results } = await db
...
export async function savePasskey(
  ...
  await db
    ...
    .bind(id, userId, credentialId, publicKey, counter, name, transports, now)
    .run();
  return (await db
...
export async function saveChallenge(
  ...
  await db
    ...
    .bind(id, userId, challenge, type, Date.now() + 5 * 60 * 1000)
    .run();
  return id;
}
...
export async function consumeChallenge(
  ...
  const row = await db
    ...
    .bind(id, Date.now())
    .first<{ challenge: string; user_id: string | null; type: string }>();
  if (row)
    await db
...
export async function saveRecoveryCodes(
  ...
  const stmts = hashes.map((hash) =>
    db
      .prepare(
        "INSERT INTO recovery_codes (id,user_id,code_hash,used,created_at) VALUES (?,?,?,?,?)",
      )
...
export async function markRecoveryCodeUsed(
  ...
  await db
    .prepare("UPDATE recovery_codes SET used=1 WHERE id=?")
    .bind(id)
    .run();
}
