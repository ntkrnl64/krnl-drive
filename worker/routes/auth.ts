...
import {
  verifyPassword, hashPassword, generateTOTPSecret, verifyTOTP, buildTOTPUri,
  generateRecoveryCode, hashRecoveryCode, verifyRecoveryCode
} from '../crypto.ts';
import {
  getUserByUsername, getUserById, createSession, getSession, deleteSession, promoteSession,
  getUnusedRecoveryCodes, markRecoveryCodeUsed, saveRecoveryCodes,
  saveChallenge, consumeChallenge,
  getPasskeysByUser, getPasskeyByCredentialId, savePasskey, updatePasskeyCounter, deletePasskey,
} from '../db.ts';
...
auth.post('/guest', async (c) => {
  ...
  if (!user || user.disabled || user.role !== 'guest') return c.json({ user: null });
...
auth.post('/verify-totp', async (c) => {
  ...
  if (!session || !session.pending_2fa) return c.json({ error: 'Invalid session' }, 401);
  ...
  if (!fullUser?.totp_secret) return c.json({ error: 'TOTP not configured' }, 400);
...
auth.post('/verify-recovery', async (c) => {
  ...
  for (const rc of codes) {
    if (await verifyRecoveryCode(code, rc.code_hash)) { matchId = rc.id; break; }
  }
...
auth.patch('/me', async (c) => {
  ...
  const body = await c.req.json<{ defaultShareTitle?: string | null; defaultShareDescription?: string | null; avatarUrl?: string | null }>();
  const sets: string[] = [];
  const setVals: unknown[] = [];
  if ('defaultShareTitle' in body) { sets.push('default_share_title=?'); setVals.push(body.defaultShareTitle ?? null); }
  if ('defaultShareDescription' in body) { sets.push('default_share_description=?'); setVals.push(body.defaultShareDescription ?? null); }
  if ('avatarUrl' in body) { sets.push('avatar_url=?'); setVals.push(body.avatarUrl ?? null); }

  if (sets.length > 0) {
    sets.push('updated_at=?');
    setVals.push(Date.now());
    setVals.push(user.id);
    await c.env.DB.prepare(`UPDATE users SET ${sets.join(',')} WHERE id=?`).bind(...setVals).run();
  }
...
auth.post('/change-password', async (c) => {
  ...
  await c.env.DB.prepare('UPDATE users SET password_hash=?,updated_at=? WHERE id=?')
    .bind(hash, Date.now(), session.user_id).run();
...
auth.post('/totp/disable', async (c) => {
  ...
  await c.env.DB.prepare('UPDATE users SET totp_enabled=0,totp_secret=NULL,updated_at=? WHERE id=?')
    .bind(Date.now(), session.user_id).run();
  await c.env.DB.prepare('DELETE FROM recovery_codes WHERE user_id=?').bind(session.user_id).run();
...
auth.post('/passkey/register/begin', async (c) => {
  ...
  const challengeId = await saveChallenge(c.env.DB, user.id, options.challenge, 'register');
...
auth.post('/passkey/register/complete', async (c) => {
  ...
  const { challengeId, response, name } = await c.req.json<{ challengeId: string; response: unknown; name?: string }>();
  const challengeData = await consumeChallenge(c.env.DB, challengeId);
  if (!challengeData || challengeData.user_id !== session.user_id) return c.json({ error: 'Invalid challenge' }, 400);
...
auth.post('/passkey/authenticate/begin', async (c) => {
  ...
  const challengeId = await saveChallenge(c.env.DB, userId, options.challenge, 'authenticate');
  return c.json({ options, challengeId });
});

// POST /api/auth/passkey/authenticate/complete
auth.post('/passkey/authenticate/complete', async (c) => {
  const { challengeId, response } = await c.req.json<{ challengeId: string; response: unknown }>();
  const challengeData = await consumeChallenge(c.env.DB, challengeId);
...
auth.delete('/passkeys/:id', async (c) => {
  ...
  const passkey = await c.env.DB.prepare('SELECT * FROM passkeys WHERE id=? AND user_id=?')
    .bind(id, session.user_id).first<{ id: string }>();
