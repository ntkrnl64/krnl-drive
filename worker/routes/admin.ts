...
admin.post("/users", requireAdmin, async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(id, username.trim(), passwordHash, userRole, 0, 0, now, now)
    .run();
...
admin.patch("/users/:id", requireAdmin, async (c) => {
  ...
  if ("root_folder_id" in body) {
    ...
    vals.push(body.root_folder_id ?? null);
  }
...
admin.post("/users/:id/force-login", requireAdmin, async (c) => {
  ...
  const target = await c.env.DB.prepare(
    "SELECT id, username, disabled FROM users WHERE id=?",
  )
    .bind(id)
    .first<{ id: string; username: string; disabled: number }>();
...
admin.get("/stats", requireAdmin, async (c) => {
  const [users, files, shares, uploadSessions] = await c.env.DB.batch([
    ...
    c.env.DB.prepare(
      "SELECT COUNT(*) as count, SUM(size) as totalSize FROM files WHERE type='file'",
    ),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM shares"),
    c.env.DB.prepare(
      "SELECT COUNT(*) as count FROM upload_sessions WHERE status='pending'",
    ),
  ]);
  ...
  return c.json({
    ...
    totalSize:
      (files.results[0] as { totalSize: number | null }).totalSize ?? 0,
