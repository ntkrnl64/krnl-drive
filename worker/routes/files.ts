...
import {
  listFiles, getFile, createFolder, updateFile, deleteFileRecord, createFile
} from '../db.ts';
...
files.post('/folder', requireAuth, async (c) => {
  ...
  if (parentId) {
    ...
    if (!parent || parent.type !== 'folder') return c.json({ error: 'Invalid parent' }, 400);
  }

  const folder = await createFolder(c.env.DB, name.trim(), parentId ?? null, user.id);
...
async function collectR2Keys(db: D1Database, file: Awaited<ReturnType<typeof getFile>>, keys: string[]) {
  if (!file) return;
...
files.post('/:id/copy', requireAuth, async (c) => {
  ...
  while (currentParent) {
    if (currentParent === fileToCopy.id) return c.json({ error: 'Cannot copy a folder into itself' }, 400);
    const parentFolder = await getFile(c.env.DB, currentParent);
    currentParent = parentFolder?.parent_id ?? null;
  }

  const copyRecursive = async (sourceFile: Awaited<ReturnType<typeof getFile>>, targetParentId: string | null): Promise<Awaited<ReturnType<typeof getFile>>> => {
    ...
    if (sourceFile.type === 'file' && sourceFile.r2_key) {
      ...
      const newFolder = await createFolder(c.env.DB, sourceFile.name, targetParentId, user.id);
