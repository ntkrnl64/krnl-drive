...
app.get("/api/share/:token", async (c) => {
  ...
  const title =
    share.custom_title ||
    creator?.default_share_title ||
    settings.default_share_title ||
    file.name;
  const description =
    share.custom_description ||
    creator?.default_share_description ||
    settings.default_share_description ||
    "";
...
app.get("/api/share/:token/browse", async (c) => {
  ...
  return c.json({
    items: items.map(({ r2_key: _, owner_id: __, ...item }) => item),
  });
});
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    return new Response("Build the frontend first: npm run build", {
      status: 404,
    });
  },
};
